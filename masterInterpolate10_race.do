...
* census10sf1_blk10_aage19 - in 2010 blk fips (blk10fips), 19 age categories for all (both sexes)
* census10sf1_blk10_mage19 - in 2010 blk fips (blk10fips), 19 age categories for males alone
* census10sf1_blk10_fage19 - in 2010 blk fips (blk10fips), 19 age categories for females alone

* create the same files for (1) Hispanic alone, (2) Non-Hispanic White alone, and (3) All the others. 
* input data: Ilgeo2010_CookDupage_P012races
* input data above was from 2010 Census FTP site (il000072010.txt& il000082010.txt, which had to be modified in Unix/VIM editor)
* and then imported to ACCESS 2007, formatted and then exported to Excel and then .DTA in Stattransfer
* (1) P012H0xx: Hispanic (or Latino) variables
* (2) P012I0xx: Non-Hispanic White variables:
* (3) Others variables is to be calculated based on previous files: i.e. Others = All - (Hispanic + Non-Hispanic)
clear all
/*
use Ilgeo2010_CookDupage_P012races1, clear
gen blk10fips= state+ county+ tract+ block
drop fileid- block
order blk10fips
sort blk10fips
save Ilgeo2010_CookDupage_P012races1_tmp, replace
use Ilgeo2010_CookDupage_P012races2, clear
gen blk10fips= state+ county+ tract+ block
drop fileid- block
order blk10fips
sort blk10fips
save Ilgeo2010_CookDupage_P012races2_tmp, replace
use Ilgeo2010_CookDupage_P012races1_tmp, clear
merge 1:1 blk10fips using Ilgeo2010_CookDupage_P012races2_tmp
assert _merge==3
/*
    Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                           116,416  (_merge==3)
    -----------------------------------------
*/
drop _merge
save Ilgeo2010_CookDupage_P012races, replace
*/
local races P012H0 P012I0
local vnames "Hispanic NonHispWhite"
local counter=1
foreach var of local races {
	use  Ilgeo2010_CookDupage_P012races, clear
	sort blk10fips
	local v: word `counter' of `vnames'
	keep blk10fips `var'*
	label variable	 `var'01	"`v' Total"
	label variable	 `var'02	"`v' Male: Total"
	label variable	 `var'03	"`v' Male: Under 5 years"
	label variable	 `var'04	"`v' Male: 5 to 9 years"
	label variable	 `var'05	"`v' Male: 10 to 14 years"
	label variable	 `var'06	"`v' Male: 15 to 17 years"
	label variable	 `var'07	"`v' Male: 18 and 19 years"
	label variable	 `var'08	"`v' Male: 20 years"
	label variable	 `var'09	"`v' Male: 21 years"
	...
	gen mage6=`var'08+`var'09+`var'10
	...
	drop P012*
	save Ilgeo2010_CookDupage_P012races_formatted_`v', replace
	preserve
	keep blk10fips mage*
	sort blk10fips
	save census10sf1_blk10_mage19_`v', replace
	restore
	preserve
	keep blk10fips fage*
	sort blk10fips
	save census10sf1_blk10_fage19_`v', replace
	restore
	keep blk10fips aage*
	sort blk10fips
	save census10sf1_blk10_aage19_`v', replace
	local counter=`counter'+1
}

* Create "Others" file ..
use census10sf1_sumlev101_p012_cookdupage_formatted, clear
...
use Ilgeo2010_CookDupage_P012races_formatted_Hispanic, clear
foreach var of varlist mage1-aage19{
...
use Ilgeo2010_CookDupage_P012races_formatted_NonHispWhite, clear
foreach var of varlist mage1-aage19{
...
use temp1, clear
sort blk10fips
merge 1:1 blk10fips using temp2
assert _merge==3
drop _merge
sort blk10fips
merge 1:1 blk10fips using temp3
assert _merge==3
drop _merge
...
*di "`vlist_fage'"
*di "`vlist_aage'"
order blk10fips `vlist_mage' `vlist_fage' `vlist_aage'
save Ilgeo2010_CookDupage_P012races_formatted_Others, replace

* Reshape to create a long format file
local counter=1
local races "Hispanic NonHispWhite Others"
foreach race of local races {
	use Ilgeo2010_CookDupage_P012races_formatted_`race', clear
	keep if substr(blk10fips,3,3)=="031"
	preserve
	keep blk10fips mage*
	sort blk10fips
	reshape long mage, i(blk10fips) j(agegpn)
	gen racegpn=`counter'  //1:HISPANIC, 2:NON-HISPANIC WHITE, 3:NON-HISPANIC OTHER
	gen sex=2 //Male
	rename mage count
	order blk10fips count agegpn racegpn sex
	save census10sf1_blk10_mage19_`race'_long, replace
	restore

	preserve
	keep blk10fips fage*
	sort blk10fips
	reshape long fage, i(blk10fips) j(agegpn)
	gen racegpn=`counter' //1:HISPANIC, 2:NON-HISPANIC WHITE, 3:NON-HISPANIC OTHER
	gen sex=3 //Female
	rename fage count
	order blk10fips count agegpn racegpn sex
	save census10sf1_blk10_fage19_`race'_long, replace
	restore
	
	keep blk10fips aage*
	sort blk10fips
	reshape long aage, i(blk10fips) j(agegpn)
	gen racegpn=`counter' //1:HISPANIC, 2:NON-HISPANIC WHITE, 3:NON-HISPANIC OTHER
	gen sex=1 //ALL
	rename aage count
	order blk10fips count agegpn racegpn sex
	save census10sf1_blk10_aage19_`race'_long, replace
	local counter=`counter'+1
}
local sexes "aage mage fage"
foreach sex of local sexes {
	use 		 census10sf1_blk10_`sex'19_Hispanic_long, clear
	append using census10sf1_blk10_`sex'19_NonHispWhite_long
	append using census10sf1_blk10_`sex'19_Others_long
	* add var labels and values
	label variable blk10fips "2010 census block 15-digit FIPS code"
	...
	sort blk10fips  racegpn agegpn sex
	save census10sf1_blk10_`sex'19_races_long, replace
}
